...
namespace WebApp1.Server.Controllers
{
    ...
    public class RentalsController : ControllerBase
    {
        ...
        public class RentRequest
        {
            ...
            public DateTime? DueDate { get; set; } // 返却締切日（null可）
            // ※ employee_name は TRN_RENTAL には保存しない（表示は JOIN で出す）
        }
        ...
        // ===== 一覧（貸出状況） =====
        // GET /rentals/list?keyword=DELL&onlyBorrowed=true
        [HttpGet("list")]
...
SELECT
  ...
  d.os,
  d.location,
  COALESCE(r.available_flag, TRUE) AS available,     -- ★空き=TRUE
  r.employee_no,
  u.employee_name,                                   -- ★氏名は MST_USER から
  r.rental_date,
  r.return_date,
  r.due_date
FROM mst_device d
...
";
  ...
  OR COALESCE(u.employee_name,'') ILIKE @kw
)");
            }
...
SELECT
  ...
  COALESCE(r.available_flag, TRUE) AS available,      -- ★
  r.employee_no, u.employee_name,                     -- ★
...
";
        ...
        // ===== 貸出 =====
        // POST /rentals/rent  body: { assetNo, employeeNo, dueDate }
        [HttpPost("rent")]
        public async Task<IActionResult> Rent([FromBody] RentRequest req)
        {
            ...
            if (string.IsNullOrWhiteSpace(req.EmployeeNo))
                return BadRequest(new { message = "社員番号は必須です" });
            ...
            using (var chk = new NpgsqlCommand(
                "SELECT 1 FROM mst_device WHERE asset_no=@a AND delete_flag=FALSE",
                _conn, (NpgsqlTransaction)tx))
            {
            ...
            bool? isFree = null;
            int? rentalId = null;
            using (var cur = new NpgsqlCommand(
                "SELECT rental_id, available_flag FROM trn_rental WHERE asset_no=@a FOR UPDATE",
                _conn, (NpgsqlTransaction)tx))
            {
                cur.Parameters.AddWithValue("a", asset);
                using var r = await cur.ExecuteReaderAsync();
                if (await r.ReadAsync())
                {
                    rentalId = r.GetInt32(0);
                    isFree = r.IsDBNull(1) ? (bool?)true : r.GetBoolean(1);
                }
            }
            if (isFree == false) return Conflict(new { message = "この機器は既に貸出中です" });

            if (rentalId == null)
            {
                // 新規 INSERT
                var ins = @"
INSERT INTO trn_rental(asset_no, employee_no, rental_date, return_date, due_date, available_flag)
VALUES (@a, @eno, CURRENT_DATE, NULL, @due, FALSE)";
                using var cmd = new NpgsqlCommand(ins, _conn, (NpgsqlTransaction)tx);
...
UPDATE trn_rental
 ...
 WHERE rental_id     = @id";
                using var cmd = new NpgsqlCommand(upd, _conn, (NpgsqlTransaction)tx);
                cmd.Parameters.AddWithValue("eno", req.EmployeeNo.Trim());
                cmd.Parameters.AddWithValue("due", (object?)req.DueDate ?? DBNull.Value);
        ...
        // ===== 返却 =====
        // POST /rentals/return  body: { assetNo }
        [HttpPost("return")]
